...
//const { user } = require('../routes/tourRoutes');
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new AppError(
        'You are not logged in! Please log in to access your app.',
        401
      )
    );
  }
  ...
  //4) Checkif user changed password ater the token was issued
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordResetExpires = undefined;
  await user.save();
  //3) Update changedPasswordExpires property for current user
